      ...
      >
        <div class="disp-space-bet" style="color: #fae8a6">
          ...
          <div class="font-we-bold">
            <div style="font-size: 20px; margin-bottom: 0.28rem;">订单取消</div>
            <!-- 付款超时时间 -->
            <div
              v-if="this.plasstime <= 60"
              style="font-size: 16px; width: 5.72rem"
            >
              超出{{ this.plasstime }}分钟未付款，订单已被系统自动取消
            </div>
            <div v-else style="font-size: 16px; width: 5.72rem">
              超出{{ this.plasstime / 60 }}小时未付款，订单已被系统自动取消
            </div>
          </div>
  ...
  methods: {
    ...
    async obine() {
      ...
      await GetMerchantPhone(this.$route.params.id).then((res) => {
        ...
        console.log(res);
        this.plasstime = res.data.data.validTime;
...
<style>
.back-imageinfo {
  background-image: url(/img/eback2.7a246424.png);
